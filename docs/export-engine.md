...
flowchart TB
  ...
  Registry --> Collect["Per-type collectors"]
...
5. Write `documentation/{type}/{id}__{name}.md` with extracted summary + raw payload
...
`manifest/dr-manifest.json` records `bundle_version` (`2.0`), Jamf Pro version, deployment mode (`cloud` / `on_prem`), and `tiers_completed`.
...
The app reports success or partial success in the UI. A backup with some HTTP errors is still useful — check `gaps/manual-workarounds.md` and `logs/failures.json`.
